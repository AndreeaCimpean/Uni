...
class MovieService:
    ...
    def update_movie(self, movieId, title, description, genre):
        ...
        params:
            ...
            genre - the new genre
        '''
        movieBefore = self.get_movies()[self._movieRepository.find_by_id(movieId)]
    ...
    def search_movie_by_id(self, movieId):
        def id_filter(m):
            return movieId in m.movieId.lower()
        movies = filter(self.get_movies(), id_filter)
        return movies

    def search_movie_by_title(self, title):
        def title_filter(m):
            return title in m.Title.lower()
        movies = filter(self.get_movies(), title_filter)
        return movies

    def search_movie_by_description(self, description):
        def description_filter(m):
            return description in m.Description.lower()
        movies = filter(self.get_movies(), description_filter)
        return movies

    def search_movie_by_genre(self, genre):
        def genre_filter(m):
            return genre in m.Genre.lower()
        movies = filter(self.get_movies(), genre_filter)
        return movies
...
class ClientService:
    ...
    def update_client(self, clientId, name):
        ...
        params:
            ...
            name - the new name
        '''
        clientBefore = self.get_clients()[self._clientRepository.find_by_id(clientId)]
    ...
    def search_client_by_id(self, clientId):
        def id_filter(c):
            return clientId in c.clientId.lower()
        clients = filter(self.get_clients(), id_filter)
        return clients

    def search_client_by_name(self, name):
        def name_filter(c):
            return name in c.Name.lower()
        clients = filter(self.get_clients(), name_filter)
        return clients
...
class RentalService:
    ...
    def find_movie_rentals(self, movieId):
        def movie_filter(r):
            return r.movieId == movieId
        rentals = filter(self.get_rentals(), movie_filter)
        return rentals

    def find_client_rentals(self, clientId):
        def client_filter(r):
            return r.clientId == clientId
        rentals = filter(self.get_rentals(), client_filter)
        return rentals
    ...
    def most_rented_movies(self):
        ...
        result = comb_sort(result, self.compare_days)
        return result
