...
int Repository::find_evidence_index_by_id(const std::string& id) const
{
...
Evidence Repository::next_evidence()
{
	Evidence evidence_before = *evidencesIterator;
	evidencesIterator++;
	if (evidencesIterator == evidences.end())
		evidencesIterator = evidences.begin();
	return evidence_before;
}



Evidence& Repository::get_evidence_by_id(const std::string& id) const
{
	int evidence_index = this->find_evidence_index_by_id(id);
	if (evidence_index == -1)
		throw "Id does not exist!";
	return this->evidences.get_element(evidence_index);
}
