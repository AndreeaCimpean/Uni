...
public:
	Repository() { this->evidencesIterator = 0; };
	virtual void add_evidence(const Evidence& evidence) = 0;
	virtual int find_evidence_by_id(const std::string& id) const = 0;
	virtual void update_evidence(const std::string& id, const std::string& measurement, const double imageClarityLevel, const double quantity, const std::string& photograph) = 0;
