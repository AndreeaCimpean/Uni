...
void UI::run() 
{
	TextFileRepository physicalCopiesRepositoryCSV{};
	HTMLRepository physicalCopiesRepositoryHTML{};
	string mode;
	string command = "";
	cin >> command;
	while (command != "exit")
	{
		...
		else if (command == "mylistLocation")
		{
			...
			if (file.find(".csv") != std::string::npos || file.find(".txt") != std::string::npos)
